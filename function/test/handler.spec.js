...
describe("Url metadata scrapper", () => {
    it("should return the metadata of the given url", (done) => {
        ...
        urlMetadataScrapper({body: {url: "https://zonehaven.com"}})
        .then ((result) => {
            expect(result).to.be.a("object");
            expect(result).to.has.property("body");
            let obj = JSON.parse(result.body);
            expect(obj).to.has.property("data");
            expect(obj.data).to.has.property("title");
            expect(obj.data).to.has.property("description");
            done();
        }).catch(done);
    });
    ...
    it("should return the bad request error", (done) => {
        // const getStub = Sinon.stub(request, 'get').resolves({url: "", body: ``})
        urlMetadataScrapper({body: {data: "https://zonehaven.com"}})
        .then ((result) => {
            expect(result).to.be.a("object");
            expect(result).to.has.property("body");
            let obj = JSON.parse(result.body);
    ...
    it("should handle the failed cases", (done) => {
        // const getStub = Sinon.stub(request, 'get').resolves({url: "", body: ``})
        urlMetadataScrapper({body: {url: "https://zonehsfefaven.com"}})
        .then ((result) => {
            expect(result).to.be.a("object");
            expect(result).to.has.property("body");
            let obj = JSON.parse(result.body);
